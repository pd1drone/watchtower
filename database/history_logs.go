...
func GetWaterLevelHistory(db sqlx.Ext) ([]*WaterLevelHistory, error) {
	...
	rows, err := db.Queryx(`SELECT ID, WaterLevel,Timestamp FROM HistoryLogs ORDER BY Timestamp DESC`)
	if err != nil {
		return nil, err
	}
